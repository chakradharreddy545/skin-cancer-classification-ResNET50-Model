...
def preprocess_image(img, target_size=(224, 224)):
    ...
    # Note: For production, ensure this is robust.
    # Using simple inpainting can sometimes introduce artifacts.
    try:
...
def get_download_link(pdf_data, filename="skin_lesion_report.pdf", text="Download PDF Report"):
    ...
    href = f'<a href="data:application/pdf;base64,{b64_pdf}" download="{filename}" class="button-download">{text}</a>'
    return href
...
def main():
    # Header
    st.markdown('<div class="main-header">Advanced Skin Cancer Classification</div>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; color: #555;">Empowering early detection with Deep Learning</p>', unsafe_allow_html=True)
    ...
    st.sidebar.info(
        "This application uses deep learning (ResNet50) to classify skin lesions into 7 diagnostic categories. "
        "It provides explainable AI features using Grad-CAM visualization."
    )
    ...
    use_tta = st.sidebar.checkbox("Use Test-Time Augmentation (TTA)", value=True, help="Augments the image multiple times for more robust predictions.")
    ...
    st.sidebar.markdown(
        ...
        4. **Check** the Grad-CAM visualization to see what the model saw.
    ...
    if uploaded_file is not None:
        ...
        with col1:
            st.markdown('<div class="sub-header">Input Image</div>', unsafe_allow_html=True)
        ...
        with st.spinner("Analyzing image..."):
            ...
            try:
                ...
                with col2:
                    st.markdown('<div class="sub-header">Explainable AI Focus</div>', unsafe_allow_html=True)
                    st.image(superimposed_img_display.astype(np.uint8), caption="Grad-CAM Visualization", use_column_width=True)
            except Exception as e:
                st.warning(f"Could not generate Grad-CAM visualization: {e}")
                superimposed_img_display = None
        ...
        with res_col1:
            ...
            <div class="diagnosis-box">
                <h3 style="color: #008080; margin: 0;">{predicted_class}</h3>
                <p style="font-size: 1.1rem; margin: 5px 0;">Confidence Score: <strong>{confidence:.2%}</strong></p>
            </div>
            """, unsafe_allow_html=True)

            with st.expander(f"Learn more about {predicted_class}"):
                st.write(class_descriptions.get(predicted_class, "No description available."))
                st.info("Consult a dermatologist for a professional diagnosis.")

        with res_col2:
            st.markdown("**Probability Distribution**")
            # Using a nicer dataframe for the chart
            prob_df = pd.DataFrame({
                'Condition': class_names,
                'Probability': predictions
            }).sort_values(by='Probability', ascending=True)

            st.bar_chart(prob_df.set_index('Condition'), height=250)

        # Feedback and Report
        st.markdown("---")
        fb_col, rep_col = st.columns(2)
        ...
        with fb_col:
            ...
            additional_feedback = st.text_input("Comments (optional)")
            full_feedback = f"Helpfulness: {feedback}\nComments: {additional_feedback}"

        with rep_col:
            st.subheader("Report")
            if st.button("Generate PDF Report"):
                if superimposed_img_display is not None:
                    gradcam_img = superimposed_img_display.astype(np.uint8)
                else:
                    gradcam_img = Image.new('RGB', (224, 224), color='gray')

                # Denormalize original processed image for the report
                mean = np.array([0.485, 0.456, 0.406])
                std = np.array([0.229, 0.224, 0.225])
                image_display = std * processed_img + mean
                image_display = np.clip(image_display, 0, 1) * 255

                pdf_data = create_report(
                    image_display.astype(np.uint8),
                    gradcam_img,
                    predicted_class,
                    confidence,
                    predictions,
        ...
        st.warning(
            "**Disclaimer:** This tool is for educational purposes only. It is not a medical device. "
            "Please consult a certified dermatologist for any skin concerns."
        )
